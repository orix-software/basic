...
;#print *
.if * > $e735
	.error .sprintf("*** ERROR GetTapeByte too long: $%X", *)
.endif
...
;#print *
.if * > $e75a
	error .sprintf("*** ERROR SyncTape too long: $%X", *)
.endif
...
;#print *
.if * > $e7af
	.error .sprintf("*** ERROR CheckFoundName too long: $%X", *)
.endif
...
;#print *
.if * > $fa86
	.error .sprintf("*** ERROR NORAMCHECK too long: $%X", *)
.endif
...
.if NO_CHARSET
	...
	;			plp
			...
			loop:
				...
				lda KBDWAIT		; Initialise le compteur pour la répétition
				sta $020e
			;fin:
				pla			; Oublie l'adresse de retour
				pla
	...
	.if ADD_DEF_CHAR
		.out "Add experimental DEF CHAR instruction"
		.define Token_USR  $D9
		...
		.if 1
			...
			Erreur:
				jmp SYNTAX_ERROR				; Renvoyer une erreur 27001 si ce qui suit le LET n'est pas valide
			.endscope
	...
	.if ADD_ONERROR
		;---------------------------------------------------------------------------
		; BUG: A=1:A=A/0 => A=10
		; Correction: sauvegarder VAR_name ($B4-B5) et/ou $B6-$B7
		;---------------------------------------------------------------------------
		SetBasicVar:
			.scope
	...
	.if ADD_ONERROR
		.out "Add experimental ON ERROR instruction"
	...
	.if * > KeyCodeTab
		.error .sprintf("*** ERROR Charset too long: $%X", *)
	.endif
	;			.res KeyCodeTab-*,$ff
...
;---------------------------------------------------------------------------
.if ROM_122
	.out "Patchs ROM 1.2x activés:"

		;---------------------------------------------------------------------------
		; Ajoute l'instruction RESTORE n
		;---------------------------------------------------------------------------
	.out "    RESTORE n (_PATCHRESTN)"
		new_patchl $c03a,2
				.word _PATCHRESTN-1
	...
	.out "    Améliorations DRAW"
		...
		new_patchl $f05a,12
			.scope
				...
				STA divid
				TXA
				JSR LEEFC::_DIVA0D	; A=X/6 : quick div
				CLC
				JMP $F06E
			.endscope
		...
		; Correction bug
		;---------------------------------------------------------------------------
	.out "    Bug arrondir quotient ($EFFA...)"
		new_patchl LEFFA,7
				; pha
				; asl $0200		; $0200.w x2
				; rol $0201
...
.if ROM_122
	...
	.endif
		...
		;---------------------------------------------------------------------------
	.if ADD_ERROR
	.out "Add experimental ERROR instruction"
			ERROR:
			.scope
				; 19 Octets
				jsr GetByteExpr+3
	...
	.if * > $fbcf
		.error .sprintf("*** ERROR PATCH SOUND too long: $%X", *)
	.endif
...
.if ADD_ONERROR
	...
	new_patch $efb1,LEFC7
		...
		LEFC7:
	.if * > $efc7
		.error .sprintf("*** ERROR PATCH ON ERROR (EFB1) too long: $%X", *)
	.endif
.endif
